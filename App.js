...
import { ActivityIndicator, Animated, BackHandler, Dimensions, Image, Linking, PanResponder, PermissionsAndroid, Platform, SafeAreaView, StatusBar, Text, TouchableOpacity, View } from "react-native";
...
const panResponder = useRef(
  PanResponder.create({
    ...
    onPanResponderMove: (event, gestureState) => {
      ...
      myWebWiew.current?.injectJavaScript(`window.scrollTo(window.scrollX - ${gestureState.dx}, window.scrollY);`);
    },
    onPanResponderRelease: (event, gestureState) => {
      ...
      if(dx >100){
        myWebWiew?.current?.goBack();
      }  if(dx < -100){   myWebWiew?.current?.goForward();}
...
const requestPhotoLibraryPermission = async () => {
  ...
  const platformPermissions3 = await request(PERMISSIONS.IOS.PHOTO_LIBRARY);
...
const onMessageReceived =async event => {
 //   console.log(event.nativeEvent.data)
  ...
  return (
    <SafeAreaView  overScrollMode="never" style={{ flex: 1, backgroundColor:"white" }} 
   // {...panResponder.panHandlers } 
    >
      ...
      {webloading ? (
                         ...
                         <View
                           ...
                           {index === 0 ? (
                             <View style={{alignItems:"center"}}>
                              {lang === "ko" ? <Bubble1SVG />:<Bubble1enSVG/>} 
                               <Image
                                 style={{width: w*0.8, height: w*0.8, marginVertical:20 }}
                                 source={item.uri}
                                 resizeMode="contain"
                               />
                               {lang === "ko" ?  <Text1SVG/>:<Text1enSVG/>}
                            ...
                            <View style={{alignItems:"center"}}>
                             {lang === "ko" ?  <Bubble2SVG />:<Bubble2enSVG/>}
                            <Image
                              style={{width: w*0.8, height: w*0.8, marginBottom:20, marginTop:10}}
                              source={item.uri}
                              resizeMode="contain"
                            />
                           {lang === "ko" ? <Text2SVG/>:<Text2enSVG/>}
        ...
        />}</View>: <View overScrollMode="never" style={{flex:1}}><WebView
        style={{flex:1}}
        ref={myWebWiew}
        originWhitelist={["*"]}
        source={{ uri: sourceUrl }}
